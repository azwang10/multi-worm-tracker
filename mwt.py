...
def make_scratch_dir(input_avi, scratch_path):
	"""Makes a scratch directory to fill with processing files"""
...
def make_mask(input_avi, scratch_dir):
	"""Makes a mask that is the median of all frames spaced out at one minute intervals"""
	...
	if os.path.isfile(mask_path):
		...
		# This ffmpeg command gets tifs 180 frames apart, which is 3 fps * 60 s
...
def guess_threshold(scratch_dir, mask, threshold_guesses=range(2, 11), size_range=(50, 150)):
	"""Guesses a threshold that will be applied to every frame"""
	# Subtracts each frame from the mask and reshapes the stack into a single row
	...
	# Quickly gets the size of each blob, filtering out blobs less than the size_range minimum
	...
	# Gets the threshold that produces blobs closest to the mean size
...
def detect_worms(scratch_dir, mask, best_threshold, size_range=(50, 150), ecc_range=(0.5, 1)):
	"""Finds worms in each frame"""
	worm_dir = scratch_dir + 'worms/'
	os.makedirs(worm_dir, exist_ok=True)

	# For each tif file, subtracts the mask, thresholds, filters for worms in size_range, and saves their positions
...
def link_worms(scratch_dir, max_speed=0.6):
	"""Links worm positions between frames"""
	...
	if os.path.isfile(untrimmed_path):
		...
		# Links two consecutive dfs in the track directory
		...
		def link(index, track_list):
			...
			# Gets the last frame of first df and the first frame of second df
			...
			# Gets a dict of ids to be replaced, every worm from the first df should only link to one worm from the second df
...
def trim_csv(scratch_dir, min_secs_tracked=30, min_area_tracked=2):
	"""Trims out worms that have not traveled far enough or were not tracked for long enough"""
...
def plot_tracks(scratch_dir, mask):
	"""Plots all tracks overlaid onto the mask"""
...
def save_output(input_avi, scratch_dir, output_path, best_threshold, size_range, ecc_range, max_speed, min_secs_tracked, min_area_tracked):
	"""Save all the outputs to the output_path"""
